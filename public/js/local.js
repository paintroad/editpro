(function initLocalModule() {
  ...
  const messageEl = document.getElementById("localMessage");
  ...
  function getSettings() {
    return { folderPath: currentFolder, ...getRenameOptions() };
  }
  ...
  function renderPreview(plan) {
    ...
    if (plan.length === 0) {
      previewBody.innerHTML =
        '<tr class="empty-row"><td colspan="4">This folder has no files to rename.</td></tr>';
      updateControls();
      return;
    }
    ...
    previewBody.innerHTML = plan
      .map((item, index) => {
        ...
        return `<tr class="${unchanged ? "unchanged" : ""}">
          <td>${index + 1}</td>
          <td>${EditProUtils.escapeHtml(item.oldName)}</td>
          <td class="arrow">${unchanged ? "—" : "→"}</td>
          <td class="new-name">${EditProUtils.escapeHtml(item.newName)}</td>
        </tr>`;
      })
      .join("");
  ...
  async function applyFolderPath() {
    ...
    try {
      const data = await EditProUtils.apiPost("/api/set-folder", { folderPath });
  ...
  async function refreshPreview() {
    ...
    try {
      if (currentDirHandle) {
        renderPreview(buildRenamePlanFromFiles(currentFileEntries, getRenameOptions()));
      } else {
        const data = await EditProUtils.apiPost("/api/preview", getSettings());
  ...
  async function renameFiles() {
    ...
    if (changes === 0) {
      EditProUtils.showMessage(messageEl, "All files already match the target names.", "warning");
      return;
    }
    ...
    try {
      if (currentDirHandle) {
        ...
        renderPreview(buildRenamePlanFromFiles(files, getRenameOptions()));
        EditProUtils.showMessage(
          messageEl,
        ...
        const data = await EditProUtils.apiPost("/api/rename", getSettings());
