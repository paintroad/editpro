const { packageDimsForSize, weightToKg } = require("./marketplace-config");
...
function deriveOrientation(widthInch, heightInch) {
...
function variantWeightKg(raw) {
  if (raw.weightKg != null && raw.weightKg !== "") {
    return Number(raw.weightKg);
  }
  const measurement = raw.inventoryItem?.measurement?.weight;
  if (measurement && measurement.value != null) {
    return weightToKg(measurement.value, measurement.unit);
  }
  if (raw.weight != null && raw.weight !== "") {
    return weightToKg(raw.weight, raw.weightUnit);
  }
  return null;
}
...
function normalizeVariant(raw, defaults = {}) {
  ...
  const packageDims = packageDimsForSize(dims.widthInch, dims.heightInch);
  ...
  return {
    ...
    orientation: deriveOrientation(dims.widthInch, dims.heightInch),
    packageDims,
    weightKg: variantWeightKg(raw),
    weightUnit: raw.weightUnit || "",
    cost: raw.cost != null ? Number(raw.cost) : null,
  };
}

function deriveProductGeometry(variants) {
  const withDims = (variants || []).find((v) => v.widthInch && v.heightInch);
  if (!withDims) {
    return { orientation: "", shape: "" };
  }
  const shape = withDims.widthInch === withDims.heightInch ? "square" : "rectangle";
  return { orientation: withDims.orientation || "", shape };
}
...
function normalizeShopifyProduct(product, ctx = {}) {
  ...
  const geometry = deriveProductGeometry(variants);
  ...
  return {
    ...
    imageUrls,
    variants,
    orientation: product.orientation || geometry.orientation,
    shape: product.shape || geometry.shape,
