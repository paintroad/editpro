...
const { filterProductsForExport, exclusionWarning } = require("./export-filters");
...
function applyExportFilters(rawProducts, source) {
  const { products, excluded } = filterProductsForExport(rawProducts, source);
...
async function inspectMarketplace({
  ...
  const { products: filteredProducts, excludedProducts } = applyExportFilters(rawProducts, source);
