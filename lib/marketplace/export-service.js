...
async function inspectMarketplace({
  ...
  const normalized = normalizeProducts(rawProducts, source, ctx);
  ...
  return {
    ...
    notes: adapter.notes,
    warnings: buildWarnings(source, normalized),
...
function buildWarnings(source, products) {
  const warnings = [];
  if (source === "catalog") {
...
async function exportMarketplace({
  ...
  const warnings = buildWarnings(source, normalized);
